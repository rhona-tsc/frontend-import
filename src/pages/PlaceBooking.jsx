...
const PlaceBooking = () => {
  ...
  const isWithin28Days = daysUntilEvent != null && daysUntilEvent <= 28;
  const clientWantsFull = paymentChoice === "full" || isWithin28Days;
  ...
  const bookedActs = useMemo(() => {
    ...
    const resolveActName = (cartActKey) => {
      ...
      // 3) fallback: resolve from summary state
  ...
  // Debug: if bookedActs stays as TBC, log why
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const resolveActFromCart = async (cartActKey) => {
        ...
        if (direct) {
          const hasLineups =
            Array.isArray(direct?.lineups) && direct.lineups.length > 0;
          if (hasLineups) return direct;

          const full = await fetchFullAct(
            direct?._id || direct?.id || cartActKey,
          );
          return full || direct;
        }
        ...
        if (byLineup) {
          const hasLineups =
            Array.isArray(byLineup?.lineups) && byLineup.lineups.length > 0;
      ...
      for (const actId in cartItemsFresh) {
        ...
        for (const lineupId in cartItemsFresh[actId]) {
          ...
          actsSummary.push({
            ...
            chosenVocalists: (chosenVocalists || []).map((id) => ({
              musicianId: id,
            })),
      ...
      if (!actsSummary.length) {
        alert(
          "We couldn't build your booking summary. Please refresh and try again.",
        );
        return;
      }
      ...
      const cartUiTotalStored = Number(
        localStorage.getItem("cartUiTotalAmount") || 0,
      );
      ...
      const amountToCharge = clientWantsFull ? cartTotal : depositAmount;

      const cartDetailsSingle = [
        {
          name: clientWantsFull
            ? `Full Booking Payment (£${cartTotal.toFixed(2)} total)`
            : `Booking Deposit (£${cartTotal.toFixed(2)} total)`,
      ...
      const stripeResponse = await axios.post(endpoint, {
        ...
        totals: {
          ...
          depositAmount,
          chargeNowAmount: amountToCharge,
          payInFullNow: clientWantsFull,
          isLessThanFourWeeks: isWithin28Days,
          currency: "GBP",
        },
            ...
            <div className="mt-4 space-y-2 rounded border border-gray-200 p-4 bg-white">
              ...
              {isWithin28Days ? (
                <p className="text-sm text-gray-600">
                  This event is within 28 days, so full payment is required.
                </p>
              ) : (
                <>
